...
def model_meta_file(name): return MODEL_DIR / f"{name}.json"
def features_file(name): return MODEL_DIR / f"{name}.npz"
def ood_file(name): return MODEL_DIR / f"{name}_ood.npz"
def calibration_file(name): return REPORTS_DIR / f"{name}_calibration.csv"
def temperature_file(name): return MODEL_DIR / f"{name}_temperature.json"